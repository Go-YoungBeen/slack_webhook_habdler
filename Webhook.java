   ...
   public static String useLLMForImage(String prompt) {
    ...
    try {
        ...
        if (response.statusCode() != 200) {
            throw new RuntimeException("Image API call failed with status " + response.statusCode() + ": " + response.body());
        }

        // JSON 응답에서 URL 추출 개선
        String responseBody = response.body();
        if (responseBody.contains("\"url\":\"")) {
            int startIndex = responseBody.indexOf("\"url\":\"") + "\"url\":\"".length();
            int endIndex = responseBody.indexOf("\"", startIndex);
            if (endIndex > startIndex) {
                return responseBody.substring(startIndex, endIndex);
            }
        }
        throw new RuntimeException("Failed to extract image URL from response: " + responseBody);
    } catch (Exception e) {
        throw new RuntimeException("Error in image generation API call: " + e.getMessage(), e);
    }
}
